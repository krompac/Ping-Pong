...
Score_Row::Score_Row(Score_Entry entry, int index)
{
	int y_pos = 130 + (index * 55);

	player1_name = UI_Element(25, y_pos, 100, 50);
	player1_score = UI_Element(130, y_pos, 50, 50);
	colon = UI_Element(185, y_pos, 25, 50);
	player2_name = UI_Element(215, y_pos, 100, 50);
	player2_score = UI_Element(320, y_pos, 50, 50);
...
void Score_Row::Render(SDL_Renderer ** renderer)
{
	...
	player1_score.Render(renderer);
	player2_name.Render(renderer);
	player2_score.Render(renderer);
